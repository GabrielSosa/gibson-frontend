...
const Inspection = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            Cell: (cellInfo: any) => {
                const appraisalData = cellInfo.row.original
                return <Link to={"/appraisal/inspection-detail/"+appraisalData.appraisal_id}>
    ...
    const loadAppraisals = async () => {
        ...
        setAllAppraisals(response.data.data.filter((v: any) => v.status === "Recepcionado" || v.status === "En inspección" || v.status === "Inspeccionado").map((v: any) => {
            return {
                ...v,
                purposes: v.purposes.split(",").map((p: any) => purposeList[p]).join(", "),
                vehicle_name: v.brand + " " + v.model,
                process_name: v.status,
                process_color: v.status === "Recepcionado" ? "danger" : v.status_color
            }
        }))
        setLoading(false)
    }
    ...
    return <>
        <Row>
            <Col>
                <Card>
                    <Card.Body>
                        <h4 className="header-title">Lista de inspección</h4>
                        <Table
                            columns={columns}
                            data={allAppraisals}
                            pageSize={5}
