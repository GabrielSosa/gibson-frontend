...
const ForgetPassword = () => {
    ...
    const schemaResolver = yupResolver(
        yup.object().shape({
            email: yup.string().required(t('Por favor ingrese un correo')).email(t('Por favor ingrese un correo')),
        })
    );
    ...
    return (
        <AuthLayout bottomLinks={<BottomLink />}>
            <div className="text-center mb-4">
                <h4 className="text-uppercase mt-0 mb-3">{t('Recuperar contraseña')}</h4>
                <p className="text-muted mb-0 font-13">
                    {t(
                        "Ingrese su correo al cual se le enviarán instrucciones para recuperar su contraseña."
                    )}
                </p>
            </div>
